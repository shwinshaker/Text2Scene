...
class WeightedMeanIOU:
    ...
    def picture_simi(self, p1, p2, thresh=0.1, with_bg=True):
        ...
        while set1 and set2:
            ...
            l1_, l2_, simi_ = sorted(numerate, key=lambda x: x[-1]['overall'])[-1]
            if simi_['overall'] > thresh:
                set1.remove(l1_)
                set2.remove(l2_)
        ...
        simi = {}
        # deno = len(p1) + len(p2) - len(inter_simi)
        deno = l1 + l2 - len(inter_simi)
        for key in ['subj', 'act', 'obj', 'overall']:
            simi[key] = sum([s[key] for s in inter_simi]) / deno
        # simi = sum(inter_simi) / (len(p1) + len(p2) - len(inter_simi))
        assert(simi['overall'] <= 1.0)
        return simi
    ...
    def layer_simi(self, layer1_, layer2_):
        ...
        dic = {'subj': 0.5,
               ...
               'obj': 0.3}
        # simi = 0
        simi = {'subj': self.jaccard(layer1['subj'], layer2['subj']),
                'act': self.jaccard_soft(layer1['act'], layer2['act']),
                'obj': self.jaccard(layer1['obj'], layer2['obj'])}
        # if not with_bg:
        #     def __fil_bg(li):
        #         return [s for s in li if s.t != 'background']
        #     simi['subj'] = self.jaccard(__fil_bg(layer1['subj']),
        #                                 __fil_bg(layer2['subj']))
