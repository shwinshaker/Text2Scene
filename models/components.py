...
from tools.common import enableQuery_
...
class GraphManipulator:
    ...
    def __init__(self, g):
        ...
        self.g_base = g.copy()
        for n in self.g.get_objs(weak_only=True):
            self.g_base.G.remove_node(n)

    def get_generator(self):
        # fixed_set = self.get_triples(strong_only=True)

        for triplets in self.get_weak_triplets():
            # comb_set = [self.rectify(*tup) for tup in weak_comb]
            g_ = self.g_base.copy()
            g_.G.add_edges_from([g_.to_edge(g_.rectify(*t)) for t in triplets])
            yield g_

    def get_weak_triplets(self):
        subjs = self.g.get_subjs()
        objs = self.g.get_objs(weak_only=True)
        ...
        for o in objs:
            ...
            all_weaks = []
            for s in self.g.G.neighbors(o):
                if not self.g.is_strong(s, o) and self.g.is_subj(s):
                    all_weaks.append((s, self.g.G[s][o]['verb'], o))
            if all_weaks:
            ...
            for s in subjs:
                ...
                if s in self.g.G.neighbors(o):
                    continue

                # objects cannot flow between characters
                if self.g.is_character(s) and any([self.g.is_character(s_) for s_ in self.g.G.neighbors(o)]):
                    continue
