...
%\[
%\mathrm{sim}_L (w_1, w_2) = \begin{cases}
%	\frac{1}{2} \left[\max\limits_{w_i\in \mathcal{C}(w_1)} \mathrm{sim}_R (w_1, w_i)+\max\limits_{w_j\in \mathcal{C}(w_2)} \mathrm{sim}_R (w_j, w_2) \right], & \mathrm{if}~ \mathrm{POS}(w_1) \ne \mathrm{POS}(w_2),\\
%	
%		\frac{1}{2}, & \mathrm{if}~ \mathrm{POS}(w_1) = \mathrm{POS}(w_2),
%	\end{cases}
%\]
...
% In practice, similarity can be weighted sum of all three.
...
%inter-subject
%\[
%\mathrm{sim}_P (l_1, l_2) =\begin{cases}
%	\frac{1}{2} \left[\max_{l_i\in \mathcal{C}(l_1)} \mathrm{sim}_P (l_1, l_i)+\max_{l_j\in \mathcal{C}(l_2)} \mathrm{sim}_P (l_j, l_2) \right],\quad & \mathrm{if}~l_2 \notin \mathcal{C}(l_1),\\
%	\frac{1}{n_{l_1}n_{l_2}}\sum_{w_1\in l_1} \sum_{w_2 \in l_2} \mathrm{sim}(w_1, w_2),\quad & \mathrm{if}~l_2 \in \mathcal{C}(l_1),
%	\end{cases}
%\]
%where $\mathrm{sim}(w_1,w_2)$ is the keyword-keyword similarity.
